...
UZYCIE:
    # Na poczatku footstats.py dodaj:
    import footstats_logging
    footstats_logging.inicjalizuj()

    # Albo uruchom sam (testuje handler):
    python footstats_logging.py
...
def raport_diagnostyczny() -> dict:
    ...
    Uzycie:
        info = footstats_logging.raport_diagnostyczny()
        print(info)
    """
    raport = {
...
if __name__ == "__main__":
    ...
    for dane, opis in testy:
        ...
        status = "✓" if (wynik is not None) == (dane not in [None, {}]) else "–"
